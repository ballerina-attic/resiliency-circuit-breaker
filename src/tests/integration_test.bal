...
//
//endpoint http:Client orderServiceEndpoint {
//    targets:[
//        {
...
//    resp = check orderServiceEndpoint -> post("/", req);
...
//    //test:assertEquals(<string>()jsonResponse.Error.toString(), "Inventory Service did not
//    //respond", msg = " Error respond mismatch");
//
...
//    // Send the request to service and get the response
//    resp = check orderServiceEndpoint -> post("/", req);
//    jsonResponse = check resp.getJsonPayload();
//    // Test the responses from the service with the original test data
//    test:assertEquals(resp.statusCode, 200, msg = "Inventory service didnot respond with 200 OK signal");
